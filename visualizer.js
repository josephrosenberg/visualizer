...
// ─── Core (Planet with shader-based displacement) ────────────────────────────
...
const PLANET_VERTEX = /* glsl */ `
  ...
  void main() {
    ...
    // Spherical coordinates for pattern variation
    float theta = atan(pos.z, pos.x);
    float phi = acos(clamp(pos.y / length(pos), -1.0, 1.0));
    float t = uTime * 0.8;
    float seed = uSeed;

    // --- Bass: large slow turbulence ---
    vec3 bassCoord = n * 1.5 + vec3(t * 0.3, t * 0.2, seed);
    float bassNoise = fbm(bassCoord, 3);
    float bassDisp = max(0.0, bassNoise) * uBass * 0.5;

    // --- Mid: sharper ridges via domain-warped noise ---
    vec3 midCoord = n * 3.5 + vec3(t * 0.6 + seed * 0.7, -t * 0.4, seed * 0.3);
    float midWarp = snoise(midCoord * 0.8 + t * 0.2);
    float midNoise = snoise(midCoord + midWarp * 0.5);
    float midDisp = pow(max(0.0, midNoise), 2.0) * uMid * 0.35;

    // --- Treble: fine grain high-frequency detail ---
    vec3 trebCoord = n * 8.0 + vec3(t * 1.2 + seed * 1.3, t * 0.8, -seed * 0.5);
    float trebNoise = snoise(trebCoord);
    float trebDisp = pow(max(0.0, trebNoise), 1.5) * uTreble * 0.2;

    // --- Beat pulse: uniform expansion ---
    float beatDisp = uBeat * 0.08;

    float totalDisp = bassDisp + midDisp + trebDisp + beatDisp;
    vDisplacement = totalDisp;

    // Displace along normal
    vec3 displaced = pos + n * totalDisp;
    ...
    vec3 neighbor1Pos = pos + tangent1 * eps;
    vec3 n1 = normalize(neighbor1Pos);
    vec3 neighbor1Coord = n1 * 1.5 + vec3(t * 0.3, t * 0.2, seed);
    float n1Disp = max(0.0, fbm(neighbor1Coord, 3)) * uBass * 0.5;
    vec3 neighbor1 = neighbor1Pos + n1 * n1Disp;

    vec3 neighbor2Pos = pos + tangent2 * eps;
    vec3 n2 = normalize(neighbor2Pos);
    vec3 neighbor2Coord = n2 * 1.5 + vec3(t * 0.3, t * 0.2, seed);
    float n2Disp = max(0.0, fbm(neighbor2Coord, 3)) * uBass * 0.5;
    vec3 neighbor2 = neighbor2Pos + n2 * n2Disp;

    vec3 displacedNormal = normalize(cross(neighbor1 - displaced, neighbor2 - displaced));
...
const PLANET_FRAGMENT = /* glsl */ `
  ...
  void main() {
    ...
    float dispT = clamp(vDisplacement * 4.0, 0.0, 1.0);
    vec3 baseColor = mix(uColorCore, uColorInner, dispT);

    // Emissive glow on displaced regions
    float emissiveStrength = dispT * 0.8 + uBeat * 0.5;
